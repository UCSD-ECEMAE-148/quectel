...
def main():
    ...
    if getattr(sys, 'frozen', False):
        ...
        execute_command = "%s -m p1_runner" % os.path.basename(sys.executable)
...
Log data with the device ID "my-device":
  ...
  python3 -m p1_runner --device-id my-device

Connect to the Point One Polaris corrections service with an assigned password:

  python3 -m p1_runner --device-id my-device --polaris PASSWORD
...
Forward NMEA output from the receiver to an application on TCP port 1234:
  ...
  python3 -m p1_runner --tcp 1234
    ...
    device_group.add_argument(
        '--device-baud', type=int, default=460800,
